...
void updateViewerFromBuffers(std::vector<std::vector<cv::Vec3f>> &buffers, std::vector<std::vector<bool>> &results, cv::viz::Viz3d &viewer, std::vector<std::vector<float>> &vscanRes, std::vector<cv::viz::WPolyLine> &polyLine, cv::viz::WPolyLine &gtLine) 
{
    ...
    // std::cout<<"size is "<< polyLine.size()<< "\n";
    // for(int i=0; i < polyLine.size() ;i++){
    //     polyLine[i].setRenderingProperty(cv::viz::LINE_WIDTH, 3.0); 
    //     std::string k = "Poly Line " + std::to_string(i);
    //     viewer.showWidget( k, polyLine[0]);
    //     std::cout<<k<< "\n";
    // }
...
void pushToBuffer(std::vector<cv::Vec3f> &buffer, const std::vector<PC_Point> &pointcloud) 
{
