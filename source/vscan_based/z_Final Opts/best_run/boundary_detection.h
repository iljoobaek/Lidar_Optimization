...
public:
    Object_detection() {
        ...
        setenv("PYTHONHOME", "/home/rtml/Kamal_Workspace/Lidar_curb_detection/source/vscan_based/env/", true);
        Py_Initialize();
        if ( !Py_IsInitialized() ){
...
//std::cout << "boundary detection constructor finished\n";
    } 
    ...
    std::vector<PC_Point>& get_pointcloud();
    std::vector<int> get_result();
    std::vector<bool> get_result_bool();
    std::vector<std::vector<cv::Vec3f>> getLidarBuffers(const std::vector<PC_Point> &pointcloud, const std::vector<bool> &result);
...
private:
    ...
    float dist_between(const PC_Point &p1, const PC_Point &p2);
...
private:
    ...
    std::vector<PC_Point> pointcloud;
    std::vector<PC_Point> pointcloud_raw;
