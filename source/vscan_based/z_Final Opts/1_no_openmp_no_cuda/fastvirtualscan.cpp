...
/*
rm 1_no_openmp_no_cuda; rm -rf build; mkdir build; cd build; cmake ..; make -j8; cd ..; ./1_no_openmp_no_cuda;
*/

extern Timers timers;
std::string optType = "raw_orig";
std::string optInfo = "raw and original no openmp in fast virtual scan";
...
void FastVirtualScan::printsvs() {
	...
	for(int i =0; i<beamnum; ++i) {
		for(int j=0; j<size; ++j) {
			cout << i*size+j << ": [" << i << "] [" << j << "]: ";
			cout << svs[i][j].rotid << " " << svs[i][j].rotlength << " " << 
			svs[i][j].rotheight << " " << svs[i][j].length << " " << 
			svs[i][j].height << endl;
		}
	}
}
