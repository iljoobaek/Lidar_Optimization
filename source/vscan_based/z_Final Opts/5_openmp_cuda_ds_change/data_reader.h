...
public:
    ...
    ~LidarDataReader() {}
private:
    void laser_to_cartesian(std::vector<velodyne::Laser> &lasers, std::vector<PC_Point> &pointcloud, float theta, cv::Mat &rot, cv::Mat &trans) 
    {
        pointcloud.clear();
    ...
    std::vector<PC_Point> readFromBinary(std::string &filename)
    {
    ...
    void rotateAndTranslate(std::vector<PC_Point> &pointcloud, float tilted_angle, float sensor_height)
    {
...
public:
    ...
    void operator >>(std::vector<PC_Point> &pointcloud) 
    {
        if (type == DataType::BINARIES)
        {
